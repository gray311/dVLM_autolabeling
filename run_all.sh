...
$PY_FASTDVLM run_fast_dvlm.py 2>&1 | tee logs/fast_dvlm.log | grep -E "^\[|saved|load_time|avg_|valid_json"

echo "===== [2/3] Nemotron-Diffusion-VLM-8B (HF diffusion) ====="
$PY_NEMO run_nemotron.py 2>&1 | tee logs/nemotron.log | grep -E "^\[|saved|load_time|avg_|valid_json"

echo "===== [3/3] Qwen3-VL-8B (HF, AR control) ====="
$PY_NEMO run_qwen3vl.py 2>&1 | tee logs/qwen3vl.log | grep -E "^\[|saved|load_time|avg_|valid_json"
